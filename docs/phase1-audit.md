...
| 6 | Optimistic concurrency | PASS | Explicit aggregate `version`, mandatory caller `expectedVersion` on affected Product/Warehouse existing-aggregate commands, service-boundary enforcement, guarded writes, and omission/stale/no-op/bulk-atomicity tests are present in lifecycle services and `tests/lifecycleVersion.test.js`. Missing/invalid values are `VALIDATION_FAILED`/400 and stale values are `STALE_VERSION`/409. |
...
| 10 | Stale updates on sensitive resources are controlled. | PASS | Product update/deactivate/archive/bulk update/bulk archive and Warehouse update/deactivate/bulk update require the current `expectedVersion` at HTTP and direct-service boundaries. `tests/lifecycleVersion.test.js` covers omission, stale no-op, stale-before-state ordering, version-bearing bulk archive, and zero-write stale batches. The shared router hardens `/api/v1` and `/api` together. |
